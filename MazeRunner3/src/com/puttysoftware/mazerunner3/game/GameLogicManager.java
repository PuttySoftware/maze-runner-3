...
import com.puttysoftware.mazerunner3.maze.effects.MazeEffectManager;
import com.puttysoftware.mazerunner3.maze.objects.Empty;
import com.puttysoftware.mazerunner3.maze.objects.EmptyVoid;
import com.puttysoftware.mazerunner3.maze.objects.HotBoots;
...
import com.puttysoftware.mazerunner3.maze.objects.SlipperyBoots;
...
public class GameLogicManager {
	...
	final JFrame owner = Game.getApplication().getOutputFrame();
	...
	final AbstractCharacter player = (AbstractCharacter) m.getCell(px, py, pz, MazeConstants.LAYER_OBJECT);
	...
	if (this.actingRemotely) {
	    ...
	    b = m.rotateRadiusClockwise(m.getPlayerLocationX(), m.getPlayerLocationY(), m.getPlayerLocationZ(), r);
	}
	if (!b) {
	    this.keepNextMessage();
	    Game.getApplication().showMessage("Rotation failed!");
	}
    }
	...
	if (this.actingRemotely) {
	    ...
	    b = m.rotateRadiusCounterclockwise(m.getPlayerLocationX(), m.getPlayerLocationY(), m.getPlayerLocationZ(),
		    r);
	}
	if (!b) {
	    this.keepNextMessage();
	...
	final AbstractMazeObject acted = Game.getApplication().getMazeManager().getMazeObject(x, y, z,
	...
	do {
		...
		try {
		    proceed = nextAbove.preMoveAction(true, px + fX, py + fY, this.getObjectInventory());
		} catch (final ArrayIndexOutOfBoundsException ae) {
		    proceed = true;
		...
		try {
			...
			redrawsSuspended = this.updatePositionRelativePull(fX, fY);
		    } else {
			redrawsSuspended = this.updatePositionRelativePush(fX, fY);
		    }
		    ...
		    nextAbove.moveFailedAction(false, px, py, this.getObjectInventory());
		...
		nextAbove.moveFailedAction(false, px + fX, py + fY, this.getObjectInventory());
		this.fireStepActions();
		this.decayEffects();
		proceed = false;
	    }
	    if (redrawsSuspended && !this.checkLoopCondition(proceed, below, nextBelow, nextAbove)) {
		// Redraw post-suspend
		this.redrawMaze();
	...
	if (acted.isPushable()) {
		...
		if (groundInto.overridesDefaultPostMove()) {
			...
			this.savedMazeObject.postMoveAction(false, px, py, this.getObjectInventory());
		    }
		} else {
		    this.savedMazeObject.postMoveAction(false, px, py, this.getObjectInventory());
		}
		...
		acted.pushFailedAction(this.getObjectInventory(), fX, fY, pushX, pushY);
		this.fireStepActions();
		this.decayEffects();
	    }
	...
	if (acted.isPullable() && this.isPullInProgress()) {
		...
		acted.pullFailedAction(this.getObjectInventory(), fX, fY, pullX, pullY);
		this.decayEffects();
	    }
	...
	if (this.autoFinishEnabled) {
	    final int ssCount = this.getObjectInventory().getItemCount(new SunStone());
	...
	if (this.alternateAutoFinishEnabled) {
	    final int msCount = this.getObjectInventory().getItemCount(new MoonStone());
    ...
    private boolean checkLoopCondition(final boolean proceed, final AbstractMazeObject below,
	...
	if (this.getObjectInventory().isItemThere(new SlipperyBoots())) {
	    ...
	    return proceed && !nextBelow.hasFrictionConditionally(this.getObjectInventory(), false)
		    && this.checkSolid(this.savedMazeObject, below, nextBelow, nextAbove);
	}
    }

    private boolean checkSolid(final AbstractMazeObject inside, final AbstractMazeObject below,
	    final AbstractMazeObject nextBelow, final AbstractMazeObject nextAbove) {
	final boolean insideSolid = inside.isConditionallySolid(this.getObjectInventory());
	final boolean belowSolid = below.isConditionallySolid(this.getObjectInventory());
	boolean nextBelowSolid = nextBelow.isConditionallySolid(this.getObjectInventory());
	...
	final boolean nextAboveSolid = nextAbove.isConditionallySolid(this.getObjectInventory());
    ...
    private void fireMoveFailedActions(final int x, final int y, final AbstractMazeObject inside,
	    final AbstractMazeObject below, final AbstractMazeObject nextBelow, final AbstractMazeObject nextAbove) {
	final boolean insideSolid = inside.isConditionallySolid(this.getObjectInventory());
	final boolean belowSolid = below.isConditionallySolid(this.getObjectInventory());
	final boolean nextBelowSolid = nextBelow.isConditionallySolid(this.getObjectInventory());
	final boolean nextAboveSolid = nextAbove.isConditionallySolid(this.getObjectInventory());
	if (insideSolid) {
    ...
    private boolean checkPush(final int x, final int y, final int pushX, final int pushY,
	    final AbstractMazeObject acted, final AbstractMazeObject nextBelow, final AbstractMazeObject nextNextBelow,
	...
	if (nextBelowAccept && nextNextBelowAccept && nextNextAboveAccept) {
	    nextBelow.pushOutAction(this.getObjectInventory(), acted, px + pushX, py + pushY, pz);
	    acted.pushAction(this.getObjectInventory(), nextNextAbove, x, y, pushX, pushY);
	    nextNextAbove.pushIntoAction(this.getObjectInventory(), acted, px + pushX, py + pushY, pz);
	    nextNextBelow.pushIntoAction(this.getObjectInventory(), acted, px + pushX, py + pushY, pz);
	    return true;
	} else {
	    return false;
	}
    ...
    private boolean checkPull(final int x, final int y, final int pullX, final int pullY,
	    final AbstractMazeObject acted, final AbstractMazeObject previousBelow, final AbstractMazeObject below,
	...
	if (previousBelowAccept && belowAccept && aboveAccept) {
	    previousBelow.pullOutAction(this.getObjectInventory(), acted, px - pullX, py - pullY, pz);
	    acted.pullAction(this.getObjectInventory(), above, x, y, pullX, pullY);
	    above.pullIntoAction(this.getObjectInventory(), acted, px - pullX, py - pullY, pz);
	    below.pullIntoAction(this.getObjectInventory(), acted, px - pullX, py - pullY, pz);
	    return true;
	} else {
	    return false;
	}
	...
	AbstractMazeObject there = mm.getMazeObject(m.getPlayerLocationX() + cumX, m.getPlayerLocationY() + cumY,
	...
	if (there != null) {
	    ...
	    } while (!there.hasFrictionConditionally(this.getObjectInventory(), true));
	}
    }

    private void movePushedObjectPosition(final int x, final int y, final int pushX, final int pushY,
	    final AbstractMovableObject o, final AbstractMazeObject g) {
	final Application app = Game.getApplication();
	final Maze m = app.getMazeManager().getMaze();
	try {
		...
		g.postMoveAction(false, m.getPlayerLocationX(), m.getPlayerLocationY(), this.getObjectInventory());
	    }
	} catch (final ArrayIndexOutOfBoundsException ae) {
	    // Do nothing
	}
    ...
    public void updatePushedIntoPositionAbsolute(final int x, final int y, final int z, final int x2, final int y2,
	    final int z2, final AbstractMovableObject pushedInto, final AbstractMazeObject source) {
	final Application app = Game.getApplication();
	final Maze m = app.getMazeManager().getMaze();
	try {
	    if (!m.getCell(x, y, z, MazeConstants.LAYER_OBJECT).isConditionallySolid(this.getObjectInventory())) {
		final AbstractMazeObject saved = m.getCell(x, y, z, MazeConstants.LAYER_OBJECT);
		...
		saved.pushIntoAction(this.getObjectInventory(), pushedInto, x2, y2, z2 - 1);
		this.redrawMaze();
		app.getMazeManager().setDirty(true);
	    }
	} catch (final ArrayIndexOutOfBoundsException ae) {
	...
	try {
	    ...
	    final AbstractMazeObject below = m.getCell(m.getPlayerLocationX(), m.getPlayerLocationY(),
		    m.getPlayerLocationZ(), MazeConstants.LAYER_GROUND);
	    final AbstractMazeObject nextBelow = m.getCell(m.getPlayerLocationX() + x, m.getPlayerLocationY() + y,
		    m.getPlayerLocationZ(), MazeConstants.LAYER_GROUND);
	    final AbstractMazeObject nextAbove = m.getCell(m.getPlayerLocationX() + x, m.getPlayerLocationY() + y,
		    m.getPlayerLocationZ(), MazeConstants.LAYER_OBJECT);
    ...
    private boolean checkSolidAbsolute(final AbstractMazeObject inside, final AbstractMazeObject below,
	    final AbstractMazeObject nextBelow, final AbstractMazeObject nextAbove) {
	final boolean insideSolid = inside.isConditionallySolid(this.getObjectInventory());
	final boolean belowSolid = below.isConditionallySolid(this.getObjectInventory());
	final boolean nextBelowSolid = nextBelow.isConditionallySolid(this.getObjectInventory());
	...
	final Maze m = app.getMazeManager().getMaze();
	try {
	    m.getCell(x, y, z, MazeConstants.LAYER_OBJECT).preMoveAction(true, x, y, this.getObjectInventory());
	} catch (final ArrayIndexOutOfBoundsException ae) {
	    // Do nothing
	} catch (final NullPointerException np) {
	    // Do nothing
	...
	try {
		...
		this.savedMazeObject = m.getCell(m.getPlayerLocationX(), m.getPlayerLocationY(), m.getPlayerLocationZ(),
			MazeConstants.LAYER_OBJECT);
		app.getMazeManager().setDirty(true);
		this.savedMazeObject.postMoveAction(false, x, y, this.getObjectInventory());
		this.checkAutoFinish();
		final int px = m.getPlayerLocationX();
		final int py = m.getPlayerLocationY();
		final int pz = m.getPlayerLocationZ();
	    ...
	    m.restorePlayerLocation();
	    this.vwMgr.restoreViewingWindow();
	    Game.getApplication().showMessage("Can't go outside the maze");
	} catch (final NullPointerException np) {
	    m.restorePlayerLocation();
	    this.vwMgr.restoreViewingWindow();
	    Game.getApplication().showMessage("Can't go outside the maze");
	...
	if (levelExists) {
	    ...
	    this.resetViewingWindow();
	    this.decay();
	    this.getObjectInventory().removeAllItemsOfType(TypeConstants.TYPE_CHECK_KEY);
	    this.oiMgr.saveObjectInventory();
	...
	if (this.actingRemotely) {
	    Game.getApplication().getMazeManager().getMaze().setCell(decay, this.remoteCoords[0], this.remoteCoords[1],
	...
	if (this.actingRemotely) {
	    Game.getApplication().getMazeManager().getMaze().setCell(this.delayedDecayObject, this.remoteCoords[0],
    ...
    public void morph(final AbstractMazeObject morphInto, final int x, final int y, final int z, final int e) {
	final Application app = Game.getApplication();
	final Maze m = app.getMazeManager().getMaze();
	try {
    ...
    public void morph(final AbstractMazeObject morphInto, final int x, final int y, final int z) {
	final Application app = Game.getApplication();
	final Maze m = app.getMazeManager().getMaze();
	try {
    ...
    public void morph(final AbstractMazeObject morphInto, final int x, final int y, final int z, final String msg) {
	final Application app = Game.getApplication();
	final Maze m = app.getMazeManager().getMaze();
	try {
	    m.setCell(morphInto, x, y, z, morphInto.getLayer());
	    Game.getApplication().showMessage(msg);
	    this.keepNextMessage();
	    this.redrawMaze();
	    app.getMazeManager().setDirty(true);
	} catch (final ArrayIndexOutOfBoundsException ae) {
    ...
    public void morphOther(final AbstractMazeObject morphInto, final int x, final int y, final int e) {
	final Application app = Game.getApplication();
	final Maze m = app.getMazeManager().getMaze();
	try {
	    m.setCell(morphInto, m.getPlayerLocationX() + x, m.getPlayerLocationY() + y, m.getPlayerLocationZ(), e);
	    this.redrawMaze();
	    app.getMazeManager().setDirty(true);
	} catch (final ArrayIndexOutOfBoundsException ae) {
	    // Do nothing
	...
	Game.getApplication().showMessage("Click to set destination");
    }
	...
	try {
	    ...
	    Game.getApplication().showMessage(gameName2 + " on " + gameName1);
	    ...
	    Game.getApplication().showMessage(ev.getGameName());
	    SoundLoader.playSound(SoundConstants.SOUND_IDENTIFY);
	}
    }
    ...
    public void loadLegacyGameHook(final XLegacyDataReader mazeFile, final int formatVersion) throws IOException {
	final Application app = Game.getApplication();
	...
	if (app.getMazeManager().getLoaded()) {
		...
		this.autoFinishEnabled = app.getMazeManager().getMaze().getAutoFinishEnabled();
		...
		this.alternateAutoFinishEnabled = app.getMazeManager().getMaze().getAlternateAutoFinishEnabled();
