...
public class Shop implements ShopTypes {
	...
	if (this.type == ShopTypes.SHOP_TYPE_WEAPONS) {
		...
		final String handResult = CommonDialogs.showInputDialog(this.getGoldTotals() + "Select Hand",
			this.getShopNameFromType(), handChoices, handChoices[handDefault]);
		if (handResult == null) {
		    return false;
		}
	...
	if (this.type == ShopTypes.SHOP_TYPE_WEAPONS) {
	    ...
	    playerCharacter.offsetGold(-this.cost);
	    playerCharacter.getItems().equipArmor(playerCharacter, (Socks) this.item, true);
	} else if (this.type == ShopTypes.SHOP_TYPE_ENHANCEMENTS) {
	    playerCharacter.offsetGold(-this.cost);
