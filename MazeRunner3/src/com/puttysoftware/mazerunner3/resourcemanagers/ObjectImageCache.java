...
public class ObjectImageCache {
	...
	if (!ObjectImageCache.isInCache(name)) {
		...
		BufferedImageIcon bii1 = ObjectImageManager.getUncachedImage(baseName);
		bii1 = ImageTransformer.templateTransformImage(bii1, transformColor);
		BufferedImageIcon bii2 = ObjectImageManager.getUncachedImage(attrName);
		bii2 = ImageTransformer.templateTransformImage(bii2, attrColor);
		final BufferedImageIcon bii3 = ImageTransformer.getCompositeImage(bii1, bii2);
		ObjectImageCache.addToCache(name, bii3);
	    } else {
		BufferedImageIcon bii = ObjectImageManager.getUncachedImage(baseName);
