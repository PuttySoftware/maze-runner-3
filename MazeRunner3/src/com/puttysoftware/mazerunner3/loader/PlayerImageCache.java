...
public class PlayerImageCache {
	...
	if (!PlayerImageCache.isInCache(name)) {
	    BufferedImageIcon bii = PlayerImageLoader.getUncachedImage(name);
	...
	final CacheEntry[] tempCache = new CacheEntry[PlayerImageCache.cache.length + PlayerImageCache.CACHE_INCREMENT];
	...
	PlayerImageCache.cache[PlayerImageCache.CACHE_SIZE] = new CacheEntry(bii, name);
