...
class MapBattleGUI {
	...
	if (this.battleFrame.isVisible()) {
		...
		for (y = yView; y <= ylView; y++) {
		    ...
		    yFix = y - yView;
		    try {
			final BufferedImageIcon icon1 = bd.getBattleMaze().getBattleGround(y, x).battleRenderHook();
			final BufferedImageIcon icon2 = bd.getBattleMaze().getBattleCell(y, x).battleRenderHook();
    ...
    void redrawOneBattleSquare(final MapBattleDefinitions bd, final int x, final int y, final AbstractMazeObject obj3) {
	// Draw the battle, if it is visible
	if (this.battleFrame.isVisible()) {
	    try {
		int xFix, yFix;
		...
		xFix = y - xView;
		yFix = x - yView;
		final BufferedImageIcon icon1 = bd.getBattleMaze().getBattleGround(x, y).battleRenderHook();
		final BufferedImageIcon icon2 = bd.getBattleMaze().getBattleCell(x, y).battleRenderHook();
		final BufferedImageIcon icon3 = obj3.battleRenderHook();
		this.drawGrid.setImageCell(ImageTransformer.getVirtualCompositeImage(icon1, icon2, icon3), xFix, yFix);
		this.battlePane.repaint();
	    } catch (final ArrayIndexOutOfBoundsException ae) {
		// Do nothing
	    } catch (final NullPointerException np) {
