...
import com.puttysoftware.mazerunner3.battle.damageengines.AbstractDamageEngine;
...
import com.puttysoftware.mazerunner3.creatures.monsters.AbstractMonster;
import com.puttysoftware.mazerunner3.creatures.monsters.MonsterFactory;
...
public class MapBattleLogic extends AbstractBattle {
	...
	final AbstractMonster m = MonsterFactory.getNewMonsterInstance(true, true, true, false);
	...
	for (final BattleCharacter enemie : enemies) {
		...
		if (enemie.getTemplate() instanceof AbstractMonster) {
		    final AbstractMonster mon = (AbstractMonster) enemie.getTemplate();
		    mon.loadMonster();
		}
	    }
	}
	...
	try {
		...
		final int x = this.bd.getActiveCharacter().getTemplate().getMapAI().getMoveX();
		final int y = this.bd.getActiveCharacter().getTemplate().getMapAI().getMoveY();
		this.lastAIActionResult = this.updatePosition(x, y);
		this.bd.getActiveCharacter().getTemplate().getMapAI().setLastResult(this.lastAIActionResult);
		break;
    ...
    private void displayRoundResults(final AbstractCreature enemy, final AbstractCreature active) {
    ...
    private void computeDamage(final AbstractCreature enemy, final AbstractCreature acting) {
	// Compute Damage
	this.damage = 0;
	...
	for (int x = 0; x < this.bd.getBattlers().length; x++) {
		...
		if (this.bd.getBattlers()[x].isActive() && this.bd.getBattlers()[x].getTemplate().getX() == -1
		    ...
		    rx = randX.generate();
		    ry = randY.generate();
		    AbstractMazeObject obj = this.bd.getBattleMaze().getBattleCell(rx, ry);
		    ...
		    this.bd.getBattleMaze().setBattleCell(this.bd.getBattlers()[x], rx, ry);
		}
	    }
	}
    }
	...
	if (res != -1) {
		...
		// No AI
		SoundLoader.playSound(SoundConstants.SOUND_PLAYER_UP);
	    }
	    return false;
	} else {
	    // Reset Speed Array
	    ...
	    SoundLoader.playSound(SoundConstants.SOUND_NEXT_ROUND);
	...
	if (this.bd.getActiveCharacter().getCurrentAP() > 0) {
	    ...
	    final MapBattleArrowTask at = new MapBattleArrowTask(x, y,
		    this.bd.getActiveCharacter().getTemplate().getFaith().getFaithID(), this.bd);
	    at.start();
	} else {
	...
	if (hit != null && !hit.getTemplate().isAlive()) {
		...
		final int partySize = PartyManager.getParty().getActivePCCount();
		this.vsd.setExpPerMonster(this.bd.findBattler(hit.getName()) - partySize,
			hit.getTemplate().getExperience());
	    }
	    // Update Prestige
	    BattlePrestige.killedEnemy(this.bd.getActiveCharacter().getTemplate());
	    ...
	    this.bd.getBattleMaze().setBattleCell(new Empty(), hit.getX(), hit.getY());
	}
	// Check result
	final int currResult = this.getResult();
	if (currResult != BattleResults.IN_PROGRESS) {
	...
	if (next != null && nextGround != null && currGround != null) {
		...
		if (useAP && this.getActiveActionCounter() >= AbstractMazeObject.getBattleAPCost() || !useAP) {
		    // Move
		    ...
		    this.decrementActiveActionCounterBy(AbstractMazeObject.getBattleAPCost());
		...
		if (next.isOfType(TypeConstants.TYPE_BATTLE_CHARACTER)) {
			...
			if (enemy.isAlive() && enemy.getTeamID() == AbstractCreature.TEAM_PARTY
				&& enemy.getCurrentHP() <= enemy.getMaximumHP() * 3 / 10) {
			    SoundLoader.playSound(SoundConstants.SOUND_LOW_HEALTH);
			}
			// Handle enemy death
			if (!enemy.isAlive()) {
			    ...
			    bc.deactivate();
			    // Remove character from battle
			    this.bd.getBattleMaze().setBattleCell(new Empty(), bc.getX(), bc.getY());
			}
			// Handle self death
			if (!active.getTemplate().isAlive()) {
			    ...
			    active.deactivate();
			    // Remove character from battle
			    this.bd.getBattleMaze().setBattleCell(new Empty(), active.getX(), active.getY());
			    // End turn
			    this.endTurn();
			}
		    } else {
	...
	if (this.getActiveActionCounter() > 0) {
		...
		final boolean success = CombatItemChucker.selectAndUseItem(this.bd.getActiveCharacter().getTemplate(),
			this.bd.getActiveCharacter().getTeamID(), true, this.bd);
		if (success) {
		    this.bd.getActiveCharacter().modifyAP(MapBattleLogic.ITEM_ACTION_POINTS);
		}
		final int currResult = this.getResult();
		if (currResult != BattleResults.IN_PROGRESS) {
		    // Battle Done
	...
	for (int x = 0; x < this.bd.getBattlers().length; x++) {
		...
		if (!active.isAlive()) {
		    ...
		    // Remove character from battle
		    this.bd.getBattleMaze().setBattleCell(new Empty(), this.bd.getBattlers()[x].getX(),
