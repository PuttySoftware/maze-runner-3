...
import com.puttysoftware.mazerunner3.maze.objects.BlockTeleport;
import com.puttysoftware.mazerunner3.maze.objects.ChainTeleport;
import com.puttysoftware.mazerunner3.maze.objects.Destination;
...
import com.puttysoftware.mazerunner3.maze.objects.InvisibleBlockTeleport;
import com.puttysoftware.mazerunner3.maze.objects.InvisibleChainTeleport;
import com.puttysoftware.mazerunner3.maze.objects.InvisibleOneShotChainTeleport;
import com.puttysoftware.mazerunner3.maze.objects.InvisibleOneShotTeleport;
import com.puttysoftware.mazerunner3.maze.objects.InvisibleTeleport;
import com.puttysoftware.mazerunner3.maze.objects.MetalButton;
import com.puttysoftware.mazerunner3.maze.objects.MovingFinish;
import com.puttysoftware.mazerunner3.maze.objects.NWayTeleport;
import com.puttysoftware.mazerunner3.maze.objects.OneShotChainTeleport;
import com.puttysoftware.mazerunner3.maze.objects.OneShotTeleport;
import com.puttysoftware.mazerunner3.maze.objects.RandomInvisibleOneShotTeleport;
import com.puttysoftware.mazerunner3.maze.objects.RandomInvisibleTeleport;
import com.puttysoftware.mazerunner3.maze.objects.RandomOneShotTeleport;
import com.puttysoftware.mazerunner3.maze.objects.RandomTeleport;
import com.puttysoftware.mazerunner3.maze.objects.StairsDown;
import com.puttysoftware.mazerunner3.maze.objects.StairsUp;
import com.puttysoftware.mazerunner3.maze.objects.Teleport;
import com.puttysoftware.mazerunner3.maze.objects.TreasureChest;
import com.puttysoftware.mazerunner3.maze.objects.TwoWayTeleport;
...
public class MazeEditorLogic {
    ...
    private static String[] conditionalChoices = new String[] { "Edit Destination 1", "Edit Destination 2",
	...
	final AbstractMazeObject instance = app.getObjects().getNewInstanceByName(mo.getName());
	...
	try {
	    ...
	    this.updateUndoHistory(this.savedMazeObject, gridX, gridY, this.getLocationManager().getEditorLocationZ(),
		    this.getLocationManager().getEditorLocationW(), this.getLocationManager().getEditorLocationE());
    ...
    private void autoGenerateTransitions(final AbstractMazeObject instance, final int gridX, final int gridY) {
    ...
    private void autoGenerateTransitions1(final AbstractMazeObject instance, final int gridX, final int gridY) {
	// Generate transitions automatically
	if (!instance.isOfType(TypeConstants.TYPE_GENERATION_ELIGIBLE)) {
	    return;
	}
    ...
    private void autoGenerateTransitions2(final AbstractMazeObject instance, final int gridX, final int gridY) {
	// Generate transitions automatically
	if (!instance.isOfType(TypeConstants.TYPE_GENERATION_ELIGIBLE)) {
	    return;
	}
    ...
    private void autoGenerateTransitions3(final AbstractMazeObject instance, final int gridX, final int gridY) {
	// Generate transitions automatically
	if (!instance.isOfType(TypeConstants.TYPE_GENERATION_ELIGIBLE)) {
	    return;
	}
    ...
    private void autoGenerateTransitions4(final AbstractMazeObject instance, final int gridX, final int gridY) {
	// Generate transitions automatically
	if (!instance.isOfType(TypeConstants.TYPE_GENERATION_ELIGIBLE)) {
	    return;
	}
    ...
    private void autoGenerateTransitions6(final AbstractMazeObject instance, final int gridX, final int gridY) {
	// Generate transitions automatically
	if (!instance.isOfType(TypeConstants.TYPE_GENERATION_ELIGIBLE)) {
	    return;
	}
    ...
    private void autoGenerateTransitions7(final AbstractMazeObject instance, final int gridX, final int gridY) {
	// Generate transitions automatically
	if (!instance.isOfType(TypeConstants.TYPE_GENERATION_ELIGIBLE)) {
	    return;
	}
    ...
    private void autoGenerateTransitions8(final AbstractMazeObject instance, final int gridX, final int gridY) {
	// Generate transitions automatically
	if (!instance.isOfType(TypeConstants.TYPE_GENERATION_ELIGIBLE)) {
	    return;
	}
    ...
    private void autoGenerateTransitions9(final AbstractMazeObject instance, final int gridX, final int gridY) {
	// Generate transitions automatically
	if (!instance.isOfType(TypeConstants.TYPE_GENERATION_ELIGIBLE)) {
	    return;
	}
	...
	try {
	    final AbstractMazeObject mo = app.getMazeManager().getMaze().getCell(gridX, gridY,
		    this.getLocationManager().getEditorLocationZ(), this.getLocationManager().getEditorLocationE());
	    this.getLocationManager().setEditorLocationX(gridX);
	    this.getLocationManager().setEditorLocationY(gridY);
	...
	case STAIRS_UP:
	    try {
		app.getMazeManager().getMaze().setCell(new StairsDown(), this.getLocationManager().getEditorLocationX(),
			this.getLocationManager().getEditorLocationY(),
			this.getLocationManager().getEditorLocationZ() + 1, MazeConstants.LAYER_OBJECT);
	    } catch (final ArrayIndexOutOfBoundsException e) {
		// Do nothing
	    }
	    break;
	case STAIRS_DOWN:
	    try {
		app.getMazeManager().getMaze().setCell(new StairsUp(), this.getLocationManager().getEditorLocationX(),
			this.getLocationManager().getEditorLocationY(),
			this.getLocationManager().getEditorLocationZ() - 1, MazeConstants.LAYER_OBJECT);
	    } catch (final ArrayIndexOutOfBoundsException e) {
		// Do nothing
	...
	case STAIRS_UP:
	    try {
		app.getMazeManager().getMaze().setCell(new StairsDown(), this.getLocationManager().getEditorLocationX(),
			this.getLocationManager().getEditorLocationY(), z + 1, MazeConstants.LAYER_OBJECT);
	    } catch (final ArrayIndexOutOfBoundsException e) {
		// Do nothing
	    }
	    break;
	...
	case STAIRS_UP:
	    try {
		app.getMazeManager().getMaze().setCell(new Empty(), this.getLocationManager().getEditorLocationX(),
			this.getLocationManager().getEditorLocationY(), z + 1, MazeConstants.LAYER_OBJECT);
	    } catch (final ArrayIndexOutOfBoundsException e) {
		// Do nothing
	    }
	...
	if (name1.equals("Two-Way Teleport")) {
	    ...
	    final AbstractMazeObject mo2 = app.getMazeManager().getMaze().getCell(destX, destY, destZ,
		    MazeConstants.LAYER_OBJECT);
	    name2 = mo2.getName();
	...
	case TELEPORT_TYPE_N_WAY:
	    ...
	    final String resp = CommonDialogs.showInputDialog("Edit Which Destination?", "Editor", dests,
	...
	case TELEPORT_TYPE_N_WAY:
	    ...
	    app.getMazeManager().getMaze().setCell(this.nWayEdited, this.getLocationManager().getEditorLocationX(),
		    this.getLocationManager().getEditorLocationY(), this.getLocationManager().getCameFromZ(),
		    MazeConstants.LAYER_OBJECT);
	    break;
	case TELEPORT_TYPE_GENERIC:
		...
		if (input3 != null) {
			...
			if (success) {
			    ...
			    this.getViewManager().setViewingWindowLocationX(
				    0 - (this.getViewManager().getViewingWindowSizeX() - 1) / 2);
			    this.getViewManager().setViewingWindowLocationY(
				    0 - (this.getViewManager().getViewingWindowSizeY() - 1) / 2);
	...
	if (input1 != null) {
		...
		if (input3 != null) {
			...
			this.fixLimits();
			this.getViewManager()
				.setViewingWindowLocationX(0 - (this.getViewManager().getViewingWindowSizeX() - 1) / 2);
			this.getViewManager()
				.setViewingWindowLocationY(0 - (this.getViewManager().getViewingWindowSizeY() - 1) / 2);
			// Save the entire level
			app.getMazeManager().getMaze().save();
			this.checkMenus();
			// Redraw
	...
	if (x != -1 && y != -1 && z != -1 && w != -1) {
	    final AbstractMazeObject oldObj = app.getMazeManager().getMazeObject(x, y, z, e);
    ...
    private void updateUndoHistory(final AbstractMazeObject obj, final int x, final int y, final int z, final int w,
	    final int e) {
	this.engine.updateUndoHistory(obj, x, y, z, w, e);
    }

    private void updateRedoHistory(final AbstractMazeObject obj, final int x, final int y, final int z, final int w,
	    final int e) {
