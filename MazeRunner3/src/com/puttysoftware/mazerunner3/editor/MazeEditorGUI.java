...
class MazeEditorGUI {
	...
	MazeEditorGUI() {
		...
		this.containableEditorAppearances = objectList.getAllContainableObjectEditorAppearances();
		...
		this.treasurePicker.updatePickerLayout(ImageTransformer.MAX_WINDOW_SIZE);
	...
	int[] computeGridValues(final int x, final int y) {
		...
		gridX = x / ImageTransformer.getGraphicSize() + this.evMgr.getViewingWindowLocationX() - xOffset + yOffset;
		gridY = y / ImageTransformer.getGraphicSize() + this.evMgr.getViewingWindowLocationY() + xOffset - yOffset;
	...
	void updateEditorPosition(final int x, final int y, final int z, final int w, final UndoRedoEngine engine) {
		...
		if (w != 0) {
			// Level Change
			Boot.getApplication().getMazeManager().getMaze().switchLevelOffset(w);
			this.fixLimits();
			this.setUpGUI();
		}
	...
	void updateEditorPositionAbsolute(final int x, final int y, final int z, final int w, final UndoRedoEngine engine) {
	...
	void updateEditorLevelAbsolute(final int w, final UndoRedoEngine engine) {
		this.elMgr.setEditorLocationW(w);
		// Level Change
		Boot.getApplication().getMazeManager().getMaze().switchLevel(w);
		this.fixLimits();
		this.setUpGUI();
		this.checkMenus(engine);
		this.redrawEditor();
	...
	void probeObjectProperties(final int x, final int y) {
		...
		final int xOffset = this.vertScroll.getValue() - this.vertScroll.getMinimum();
		final int yOffset = this.horzScroll.getValue() - this.horzScroll.getMinimum();
		final int gridX = x / ImageTransformer.getGraphicSize() + this.evMgr.getViewingWindowLocationX() - xOffset
				+ yOffset;
		final int gridY = y / ImageTransformer.getGraphicSize() + this.evMgr.getViewingWindowLocationY() + xOffset
				- yOffset;
		try {
	...
	void handleCloseWindow() {
		try {
			final MazeEditorLogic mel = Boot.getApplication().getEditor();
			final Application app = Boot.getApplication();
