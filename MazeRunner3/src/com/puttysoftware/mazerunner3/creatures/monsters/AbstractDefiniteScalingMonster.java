...
abstract class AbstractDefiniteScalingMonster extends AbstractDefiniteMonster {
	...
	final RandomRange r = new RandomRange(0, Math.max(this.getLevel() * StatConstants.GAIN_BLOCK, 1));
	return r.generate();
    }
	...
	final int min = 0;
	final int max = this.getToughness() * AbstractMonster.GOLD_TOUGHNESS_MULTIPLIER;
	...
	final RandomRange r = new RandomRange(0, Math.max(this.getLevel() * StatConstants.GAIN_INTELLIGENCE, 1));
	return r.generate();
    }

    private int getInitialLuck() {
	final RandomRange r = new RandomRange(0, Math.max(this.getLevel() * StatConstants.GAIN_LUCK, 1));
	return r.generate();
    }
}
