...
public class Party {
	...
	for (int x = 0; x < divMod; x++) {
		...
		this.battlers[x].getTemplate()
			.offsetExperience(AbstractCreature.getAdjustedExperience(vsd.getExpPerMonster(y)) / divMod);
	    }
	...
	final String response = CommonDialogs.showInputDialog(
		"Pick " + number + " Party Member(s) - " + current + " of " + number, "MazeRunnerII", pickNames,
		pickNames[0]);
	if (response != null) {
	    final int loc = this.findMember(response, 0, this.members.length);
	    if (loc != -1) {
    ...
    static Party readLegacy(final XLegacyDataReader worldFile) throws IOException {
