...
import com.puttysoftware.mazerunner3.maze.objects.NormalAmulet;
...
public final class MazeObjectInventory implements Cloneable {
	...
	if (!MazeObjectInventory.isBoots(mo) && !MazeObjectInventory.isAmulet(mo)) {
	    if (!MazeObjectInventory.isBow(mo)) {
	...
	if (!this.boots.getName().equals(MazeObjectInventory.DEFAULT_BOOTS.getName())) {
	    this.boots.stepAction();
	}
	if (!this.amulet.getName().equals(MazeObjectInventory.DEFAULT_AMULET.getName())) {
	...
	i.amulet = (AbstractAmulet) objects.readLegacyMazeObject(reader, formatVersion);
	if (i.amulet == null) {
	...
	i.amulet = (AbstractAmulet) objects.readMazeObject(reader, formatVersion);
	if (i.amulet == null) {
	    i.amulet = MazeObjectInventory.DEFAULT_AMULET;
	}
	for (int x = 0; x < i.contents.length; x++) {
