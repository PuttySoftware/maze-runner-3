...
import com.puttysoftware.mazerunner3.maze.legacy.LegacySuffixIO;
...
public class Maze implements MazeConstants {
	...
	this.basePath = System.getProperty("java.io.tmpdir") + File.separator + "MazeRunnerII" + File.separator
		+ randomID + ".maze";
	final File base = new File(this.basePath);
	final boolean success = base.mkdirs();
	if (!success) {
	    CommonDialogs.showErrorDialog("Maze temporary folder creation failed!", "MazeRunnerII");
	}
    }

    // Static methods
    public static String getMazeTempFolder() {
	return System.getProperty("java.io.tmpdir") + File.separator + "MazeRunnerII";
    }
	...
	this.mazeData.radialScanTimerAction(x, y, z, l, r, targetName, timerMod);
    }
    ...
    public void setBattleCell(final AbstractMazeObject mo, final int row, final int col) {
    ...
    public void fillLevel(final AbstractMazeObject bottom, final AbstractMazeObject top) {
	this.mazeData.fill(bottom, top);
    }
	...
	final AbstractMazeObject fillWith = PreferencesManager.getEditorDefaultFill(layer);
    ...
    private void fillLevelRandomlyInBattle(final AbstractMazeObject pass1FillBottom,
	    final AbstractMazeObject pass1FillTop) {
	this.mazeData.fillRandomlyInBattle(this, this.activeLevel, pass1FillBottom, pass1FillTop);
    }
	...
	int version = 0;
	// Create metafile reader
	try (XLegacyDataReader metaReader = new XLegacyDataReader(m.basePath + File.separator + "metafile.xml",
		"maze")) {
	    // Read metafile
    ...
    private int readMazeLegacyMetafile(final XLegacyDataReader reader) throws IOException {
    ...
    private void readLegacyMazeLevel(final XLegacyDataReader reader, final int formatVersion) throws IOException {
