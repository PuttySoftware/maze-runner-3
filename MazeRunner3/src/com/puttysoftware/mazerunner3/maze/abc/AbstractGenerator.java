...
import com.puttysoftware.mazerunner3.maze.objects.HorizontalBarrier;
import com.puttysoftware.mazerunner3.maze.objects.VerticalBarrier;
...
public abstract class AbstractGenerator extends AbstractWall {
	...
	final AbstractMazeObject mo2 = app.getMazeManager().getMazeObject(dirX - 1, dirY, pz,
	...
	final AbstractMazeObject mo4 = app.getMazeManager().getMazeObject(dirX, dirY - 1, pz,
		MazeConstants.LAYER_OBJECT);
	try {
	    mo4Name = mo4.getName();
	} catch (final NullPointerException np) {
	...
	final AbstractMazeObject mo6 = app.getMazeManager().getMazeObject(dirX, dirY + 1, pz,
		MazeConstants.LAYER_OBJECT);
	try {
	    mo6Name = mo6.getName();
	} catch (final NullPointerException np) {
	...
	final AbstractMazeObject mo8 = app.getMazeManager().getMazeObject(dirX + 1, dirY, pz,
		MazeConstants.LAYER_OBJECT);
	try {
	    mo8Name = mo8.getName();
	} catch (final NullPointerException np) {
	...
	if (flag) {
	    SoundLoader.playSound(SoundConstants.SOUND_GENERATE);
	    this.activateTimer(this.TIMER_DELAY);
	...
	if (dir == DirectionConstants.DIRECTION_EAST) {
		...
		final AbstractMazeObject mo = app.getMazeManager().getMazeObject(x + l, y, z,
			MazeConstants.LAYER_OBJECT);
		String moName;
		try {
		    moName = mo.getName();
	    ...
	    for (int l = 1; l < limit; l++) {
		final AbstractMazeObject mo = app.getMazeManager().getMazeObject(x, y - l, z,
			MazeConstants.LAYER_OBJECT);
		String moName;
		try {
		    moName = mo.getName();
    ...
    protected abstract void arrowHitActionHook(int locX, int locY, int locZ, int arrowType, MazeObjectInventory inv);
