...
public abstract class AbstractMazeObject implements TypeConstants, RandomGenerationRule {
    ...
    public final AbstractMazeObject readLegacyMazeObjectV1(final XLegacyDataReader reader, final String ident)
	    throws IOException {
	if (ident.equals(this.getIdentifierV1())) {
	    ...
	    final int cc = this.getCustomFormat();
	    if (cc == AbstractMazeObject.CUSTOM_FORMAT_MANUAL_OVERRIDE) {
		return this.readLegacyMazeObjectHook(reader, LegacyFormatConstants.LEGACY_MAZE_FORMAT_1);
	    } else {
		for (int x = 0; x < cc; x++) {
    ...
    public final AbstractMazeObject readLegacyMazeObjectV2(final XLegacyDataReader reader, final String ident)
	    throws IOException {
	if (ident.equals(this.getIdentifier())) {
	    ...
	    final int cc = this.getCustomFormat();
	    if (cc == AbstractMazeObject.CUSTOM_FORMAT_MANUAL_OVERRIDE) {
		return this.readLegacyMazeObjectHook(reader, LegacyFormatConstants.LEGACY_MAZE_FORMAT_2);
	    } else {
		for (int x = 0; x < cc; x++) {
		    final int cx = reader.readInt();
		    this.setCustomProperty(x + 1, cx);
