...
import com.puttysoftware.mazerunner3.maze.objects.BarrierGenerator;
import com.puttysoftware.mazerunner3.maze.objects.CrackedWall;
import com.puttysoftware.mazerunner3.maze.objects.Crevasse;
...
import com.puttysoftware.mazerunner3.maze.objects.EnragedBarrierGenerator;
import com.puttysoftware.mazerunner3.maze.objects.ForceField;
import com.puttysoftware.mazerunner3.maze.objects.HotRock;
import com.puttysoftware.mazerunner3.maze.objects.Ice;
import com.puttysoftware.mazerunner3.maze.objects.IcedBarrierGenerator;
import com.puttysoftware.mazerunner3.maze.objects.LightGem;
import com.puttysoftware.mazerunner3.maze.objects.Monster;
import com.puttysoftware.mazerunner3.maze.objects.MovingBlock;
import com.puttysoftware.mazerunner3.maze.objects.MovingFinish;
import com.puttysoftware.mazerunner3.maze.objects.Player;
import com.puttysoftware.mazerunner3.maze.objects.PoisonedBarrierGenerator;
import com.puttysoftware.mazerunner3.maze.objects.ShockedBarrierGenerator;
import com.puttysoftware.mazerunner3.maze.objects.Slime;
...
class LayeredTower implements Cloneable {
	...
	try {
	    ...
	    if (!there.isSolid() && xLoc + dirMove[0] != xLocP && yLoc + dirMove[1] != yLocP) {
	...
	final int zLoc = this.getPlayerFloor();
	if (player) {
	    Boot.getApplication().getGameManager().setSavedMazeObject(saved);
	} else {
	...
	for (u = x - r; u <= x + r; u++) {
		...
		try {
			...
			Boot.getApplication().getBattle().doBattleByProxy();
			// Respawn it
			this.generateOneMonster();
		    }
		} catch (final ArrayIndexOutOfBoundsException aioob) {
    ...
    public void findAllObjectPairsAndSwap(final AbstractMazeObject o1, final AbstractMazeObject o2) {
	int y, x, z;
	for (x = 0; x < this.getColumns(); x++) {
	    for (y = 0; y < this.getRows(); y++) {
		for (z = 0; z < this.getFloors(); z++) {
		    final AbstractMazeObject mo = this.getCell(y, x, z, MazeConstants.LAYER_OBJECT);
		    if (mo != null) {
	...
	final LowLevelAMODataStore tempStorage = new LowLevelAMODataStore(y, x, z, MazeConstants.LAYER_COUNT);
	...
	this.savedTowerState = new LowLevelAMODataStore(y, x, z, MazeConstants.LAYER_COUNT);
    }
	...
	if (xDist <= this.visionRadius && yDist <= this.visionRadius) {
	    if (this.radialScanVision(x2, y2, zLoc, MazeConstants.LAYER_OBJECT, AbstractLightModifier.getEffectRadius(),
		    dg.getName())) {
		result = false;
	    } else {
		result = true;
	...
	for (x = 0; x < this.getColumns(); x++) {
	    for (y = 0; y < this.getRows(); y++) {
		for (z = 0; z < this.getFloors(); z++) {
		    final AbstractMazeObject obj = this.getCell(y, x, z, MazeConstants.LAYER_OBJECT);
	...
	final AbstractMazeObject pass1FillBottom = PreferencesManager.getEditorDefaultFill();
	...
	final AbstractMazeObject pass1FillBottom = PreferencesManager.getEditorDefaultFill();
	final AbstractMazeObject pass1FillTop = new Empty();
	final AbstractMazeObject[] withoutRuleSets = objects.getAllObjectsWithoutRuleSets();
	final AbstractMazeObject[] withRuleSets = objects.getAllObjectsWithRuleSets();
	RandomRange r = null;
	int x, y, e;
	// Pass 1
	this.fillFloor(pass1FillBottom, pass1FillTop, z);
	...
	if (withoutRuleSets != null) {
	    ...
	    // Pass 3
	    for (int layer = 0; layer < MazeConstants.LAYER_COUNT; layer++) {
		final AbstractMazeObject[] requiredObjects = MazeObjectList.getAllRequiredSubset(withoutRuleSets,
			layer);
		if (requiredObjects != null) {
		    final RandomRange row = new RandomRange(0, this.getRows() - 1);
		    final RandomRange column = new RandomRange(0, this.getColumns() - 1);
	...
	if (withRuleSets != null) {
	    ...
	    // Pass 5
	    for (int layer = 0; layer < MazeConstants.LAYER_COUNT; layer++) {
		final AbstractMazeObject[] requiredObjects = MazeObjectList.getAllRequiredSubset(withRuleSets, layer);
		if (requiredObjects != null) {
		    final RandomRange row = new RandomRange(0, this.getRows() - 1);
		    final RandomRange column = new RandomRange(0, this.getColumns() - 1);
		    int randomColumn, randomRow;
    ...
    public void fillFloorAndLayer(final AbstractMazeObject fillWith, final int z, final int e) {
	int x, y;
	for (x = 0; x < this.getColumns(); x++) {
	    for (y = 0; y < this.getRows(); y++) {
		this.setCell(fillWith, y, x, z, e);
	...
	// Pre-Pass
	final MazeObjectList objects = Boot.getApplication().getObjects();
	final AbstractMazeObject pass1Fill = PreferencesManager.getEditorDefaultFill(layer);
	RandomRange r = null;
	int x, y;
	// Pass 1
	...
	final int columns = this.getColumns();
	final int rows = this.getRows();
	final AbstractMazeObject[] objectsWithoutPrerequisites = objects.getAllWithoutPrerequisiteAndNotRequired(layer);
	if (objectsWithoutPrerequisites != null) {
	    r = new RandomRange(0, objectsWithoutPrerequisites.length - 1);
	    for (x = 0; x < columns; x++) {
		for (y = 0; y < rows; y++) {
	...
	if (withoutRuleSets != null) {
	    ...
	    // Pass 3
	    final AbstractMazeObject[] requiredObjects = MazeObjectList.getAllRequiredSubset(withoutRuleSets, layer);
	    if (requiredObjects != null) {
		final RandomRange row = new RandomRange(0, this.getRows() - 1);
		final RandomRange column = new RandomRange(0, this.getColumns() - 1);
		int randomColumn, randomRow;
    ...
    public void fillRandomlyInBattle(final Maze map, final int w, final AbstractMazeObject pass1FillBottom,
    ...
    private void fillFloorRandomlyInBattle(final Maze map, final int z, final int w,
	    final AbstractMazeObject pass1FillBottom, final AbstractMazeObject pass1FillTop) {
	// Pre-Pass
	...
	final int rows = this.getRows();
	for (e = 0; e < MazeConstants.LAYER_COUNT; e++) {
	    final AbstractMazeObject[] objectsWithoutPrerequisites = objects.getAllNotRequiredInBattle(e);
	    if (objectsWithoutPrerequisites != null) {
		r = new RandomRange(0, objectsWithoutPrerequisites.length - 1);
		for (x = 0; x < columns; x++) {
		    for (y = 0; y < rows; y++) {
			if (e == MazeConstants.LAYER_GROUND) {
				...
				for (y = 0; y < rows; y += this.regionSize) {
				    final AbstractMazeObject placeObj = objectsWithoutPrerequisites[r.generate()];
				    final boolean okay = placeObj.shouldGenerateObjectInBattle(map, y, x, z, w, e);
				    if (okay) {
	...
	// Pass 3
	for (int layer = 0; layer < MazeConstants.LAYER_COUNT; layer++) {
	    final AbstractMazeObject[] requiredObjects = objects.getAllRequiredInBattle(layer);
	    if (requiredObjects != null) {
		final RandomRange row = new RandomRange(0, this.getRows() - 1);
		final RandomRange column = new RandomRange(0, this.getColumns() - 1);
		int randomColumn, randomRow;
		for (x = 0; x < requiredObjects.length; x++) {
		    ...
		    randomColumn = column.generate();
		    final AbstractMazeObject currObj = requiredObjects[x];
		    final int min = currObj.getMinimumRequiredQuantityInBattle(map);
	...
	final AbstractMazeObject bottom = PreferencesManager.getEditorDefaultFill();
	...
	if (this.timerActive) {
		...
		Boot.getApplication().getGameManager().solvedLevel();
	    }
	}
    }
    ...
    public boolean radialScan(final int cx, final int cy, final int r, final int tx, final int ty) {
	return Math.abs(tx - cx) <= r && Math.abs(ty - cy) <= r;
    }

    public boolean radialScanVision(final int x, final int y, final int z, final int l, final int r,
	...
	for (u = x - r; u <= x + r; u++) {
		...
		try {
		    ...
		    if (reactsToIce) {
			final AbstractMazeObject there = this.getCell(u, v, z, MazeConstants.LAYER_OBJECT);
	...
	final boolean needsFix = px >= x - r && px <= x + r && py >= y - r && py <= y + r;
	...
	// Do the shuffle
	final AbstractMazeObject[][][] postShuffle = LayeredTower.shuffleObjects(preShuffle, r, px - (x - r),
		py - (y - r), needsFix);
	...
	for (u = x - r; u <= x + r; u++) {
		...
		try {
			...
			PartyManager.getParty().getLeader().doDamagePercentage(2);
		    }
		} catch (final ArrayIndexOutOfBoundsException aioob) {
		    // Do nothing
		}
    ...
    public void warpObject(final AbstractMazeObject mo, final int x, final int y, final int z, final int l) {
	final RandomRange row = new RandomRange(0, this.getRows() - 1);
	final RandomRange column = new RandomRange(0, this.getColumns() - 1);
	int randomColumn, randomRow;
	randomRow = row.generate();
	randomColumn = column.generate();
	final AbstractMazeObject currObj = this.getCell(randomRow, randomColumn, z, MazeConstants.LAYER_OBJECT);
	if (!currObj.isSolid()) {
    ...
    private static AbstractMazeObject[][][] shuffleObjects(final AbstractMazeObject[][][] preShuffle, final int r,
	...
	randomLocations = LayeredTower.shuffleArray(randomLocations, opx, opy, needsFix);
    ...
    private static int[][] shuffleArray(final int[][] src, final int opx, final int opy, final boolean needsFix) {
	int temp = 0;
	...
	final AbstractMazeObject obj = this.getCell(this.getFirstMovingFinishX(), this.getFirstMovingFinishY(),
    ...
    public static LayeredTower readLegacyLayeredTowerV1(final XLegacyDataReader reader) throws IOException {
	...
	for (x = 0; x < lt.getColumns(); x++) {
		...
		for (z = 0; z < lt.getFloors(); z++) {
		    for (e = 0; e < MazeConstants.LAYER_COUNT; e++) {
			AbstractMazeObject obj = Boot.getApplication().getObjects().readLegacyMazeObject(reader,
    ...
    public static LayeredTower readLegacyLayeredTowerV2(final XLegacyDataReader reader) throws IOException {
	int y, x, z, e, mazeSizeX, mazeSizeY, mazeSizeZ;
	mazeSizeX = reader.readInt();
	mazeSizeY = reader.readInt();
	mazeSizeZ = reader.readInt();
	...
	for (x = 0; x < lt.getColumns(); x++) {
		...
		for (z = 0; z < lt.getFloors(); z++) {
		    for (e = 0; e < MazeConstants.LAYER_COUNT; e++) {
			lt.setCell(Boot.getApplication().getObjects().readLegacyMazeObject(reader,
				LegacyFormatConstants.LEGACY_MAZE_FORMAT_2), y, x, z, e);
	...
	for (x = 0; x < lt.getColumns(); x++) {
		...
		for (z = 0; z < lt.getFloors(); z++) {
		    for (e = 0; e < MazeConstants.LAYER_COUNT; e++) {
			lt.setCell(Boot.getApplication().getObjects().readMazeObject(reader,
				FormatConstants.MAZE_FORMAT_1), y, x, z, e);
			if (lt.getCell(y, x, z, e) == null) {
			    return null;
			}
		    }
    ...
    public void readLegacySavedTowerState(final XLegacyDataReader reader, final int formatVersion) throws IOException {
	...
	for (x = 0; x < sizeY; x++) {
		...
		for (z = 0; z < sizeZ; z++) {
			...
			this.savedTowerState.setCell(
				Boot.getApplication().getObjects().readLegacyMazeObject(reader, formatVersion), y, x, z,
				e);
		    }
		}
	    }
	...
	for (x = 0; x < sizeY; x++) {
		...
		for (z = 0; z < sizeZ; z++) {
		    for (e = 0; e < MazeConstants.LAYER_COUNT; e++) {
			this.savedTowerState.setCell(
				Boot.getApplication().getObjects().readMazeObject(reader, formatVersion), y, x, z, e);
		    }
		}
	    }
	}
