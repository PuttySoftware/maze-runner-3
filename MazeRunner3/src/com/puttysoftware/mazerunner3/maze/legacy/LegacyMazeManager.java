...
public class LegacyMazeManager {
	...
	} else if (extension.equals(LegacyExtension.getLegacySavedGameExtension())) {
	...
	if (saved) {
		...
		for (int x = 0; x < choices.length; x++) {
		    choices[x] = LegacyMazeManager.getNameWithoutExtension(rawChoices[x]);
		}
    ...
    private static void loadLegacyFile(final String filename, final boolean isSavedGame, final boolean locked) {
	if (!FilenameChecker
		.isFilenameOK(LegacyMazeManager.getNameWithoutExtension(LegacyMazeManager.getFileNameOnly(filename)))) {
		...
		final LegacyLoadTask xlt = new LegacyLoadTask(filename, isSavedGame);
		xlt.start();
	    }
	}
    }
